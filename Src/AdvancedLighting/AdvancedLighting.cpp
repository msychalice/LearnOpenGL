...
int main()
{
	...
	shaderGeometryPass.load("g_buffer.vert", "g_buffer.frag");
	Shader shaderLightingPass;
	shaderLightingPass.load("deferred_shading.vert", "deferred_shading.frag");
	Shader shaderLightBox;
	shaderLightBox.load("deferred_light_box.vert", "deferred_light_box.frag");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		shaderGeometryPass.setMatrix4fv("projection", projection);
		shaderGeometryPass.setMatrix4fv("view", view);
		for (unsigned int i = 0; i < objectPositions.size(); i++)
		{
			model = glm::mat4();
			model = glm::translate(model, objectPositions[i]);
			model = glm::scale(model, glm::vec3(0.25f));
			shaderGeometryPass.setMatrix4fv("model", model);
		...
		{
			shaderLightingPass.setVector3fv(("lights[" + std::to_string(i) + "].Position").c_str(), lightPositions[i]);
			shaderLightingPass.setVector3fv(("lights[" + std::to_string(i) + "].Color").c_str(), lightColors[i]);
		...
		// --------------------------------
		shaderLightBox.use();
		shaderLightBox.setMatrix4fv("projection", projection);
		shaderLightBox.setMatrix4fv("view", view);
		for (unsigned int i = 0; i < lightPositions.size(); i++)
		{
			model = glm::mat4();
			model = glm::translate(model, lightPositions[i]);
			model = glm::scale(model, glm::vec3(0.125f));
			shaderLightBox.setMatrix4fv("model", model);
			shaderLightBox.setVector3fv("lightColor", lightColors[i]);
			glBindVertexArray(cubeVAO);
